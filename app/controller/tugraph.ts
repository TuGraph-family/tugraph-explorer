...
class TuGraphController extends Controller {
  ...
  async connect() {
    ...
    const result = await ctx.service.tugraph.connect(
      username,
      password,
      serverUrl
    );
  ...
  async getSchema() {
    ...
    const result = await ctx.service.tugraph.querySchema(
      graphName,
      authorization
    );
    responseData(ctx, result);
  ...
  async getVertexEdgeCount() {
    ...
    const result = await ctx.service.tugraph.getVertexEdgeCount(
      graphName,
      authorization
    );
    responseData(ctx, result);
